              ...
              <v-row class="form">
                ...
                <v-card-text style="width: 100%">
                  <v-row>
                    ...
                    <v-combobox v-model="selectedLigne" :items="lignesOptions" label="اختر الخط" item-text="gare_id"
                      item-value="id" v-if="lignesOptions && lignesOptions.length > 0"></v-combobox>
...
async function fetchLignes() {
  try {
    ...
    const data = response.data.Lignes;

    // Formatez les données pour lignesOptions avec les propriétés text et value
    lignesOptions.value = data.map((ligne) => ({
      text: `${ligne.station_name} - ${ligne.fin_station_name}`,
      value: ligne.id,
    }));
    console.log(lignesOptions.value)
  } catch (error) {
...
onMounted(async () => {
  ...
  await fetchLignes();
  await fetchAbn();
  console.log("fetchLignes a été appelé.");
});
