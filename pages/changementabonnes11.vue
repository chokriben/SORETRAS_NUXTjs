...
async function createSession() {
  try {
    const response = await axios.post("https://api.srtk.com.tn/api/payement", {
      ...
      ...merchant,
      ...customer.value,
      differencePrix: differencePrix, // Ajoutez cette ligne pour inclure differencePrix
      ...order.value,
    });
    if (
      response.data &&
...
const handleRegistration = async () => {
  ...
  let typePeriode = semestre.value === "sem1" ? "sem1" : "an";
  ...
  try {
    if (etat.value === "imprimer") {
      ...
      if (parseFloat(prix.value) > parseFloat(prixValue)) {
        // console.log("mmmm")
        const differencePrix = prix.value - prixValue;
