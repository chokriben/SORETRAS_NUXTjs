  ...
  <div>
    ...
    <v-form @submit.prevent="handleSubmit">
      ...
      <v-row>
        ...
        <v-col cols="12" sm="6">
          <v-text-field v-model="formData.longitude" label="Longitude" prepend-inner-icon="mdi-map-marker"
            placeholder="Enter longitude"></v-text-field>
        </v-col>
        <v-col cols="12" sm="6">
          <v-text-field v-model="formData.latitude" label="Latitude" prepend-inner-icon="mdi-map-marker"
            placeholder="Enter latitude"></v-text-field>
        </v-col>
        <v-col cols="6" sm="6">
          <v-text-field v-model="formData.code" label="Code" prepend-inner-icon="mdi-numeric"
    ...
    <div v-if="stations.length" class="centered-container">
      <h2 class="stations-title green-title">Liste des stations</h2>
      <ul>
        <li v-for="station in stations" :key="station.id" class="station-item">
          <div class="station-data">
            <h3>{{ station.name }}</h3>
            <p>{{ station.gare_id }}</p>
            <p>{{ station.ligne_id }}</p>
            <p></p>
          </div>
          ...
          <div class="station-actions">
            <v-dialog v-model="dialogVisible" max-width="500px">
              <template v-slot:activator="{ on }">
                <v-btn color="primary" v-on="on" @click="editstation(station)">Modifier</v-btn>
              </template>
              <v-card>
                <v-card-title>
            ...
            <v-btn color="error" @click="deletestation(station)">Supprimer</v-btn>
          </div>
        </li>
      </ul>
    </div>
