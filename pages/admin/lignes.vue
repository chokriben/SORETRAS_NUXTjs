...
export default {
  ...
  methods: {
    async async(search = false) {
      ...
      let url = `/admin/lignes?page=${this.currentPage}`;

      if (search) {
        url += `&search=${this.searchTerm}`; // Append the search term to the URL
      }
      axios
        .get(`/admin/lignes?page=${this.currentPage}&search=${this.searchTerm}`, {
          headers: { Authorization: `Bearer ${token.value}` },
        })
        .then((res) => {
          this.totalItems = res.data.Lignes.total;
          this.Lignes = res.data.Lignes.data.data;
          this.isReady = true;
          this.editDialog = false;
        })
        .catch((err) => {
