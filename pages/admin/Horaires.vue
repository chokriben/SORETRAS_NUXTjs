<template>
  <div>
    ...
    <v-table height="690" fixed-header>
      <thead>
        <tr>
          <th class="text-uppercase">Station depart</th>
          <th class="text-uppercase">Station arrivé</th>
          <th class="text-uppercase">heure de depart</th>
          <th class="text-uppercase">heure d'arrivé</th>
...
export default {
  ...
  methods: {
    async async(search = false) {
      ...
      let url = `/admin/horaires?page=${this.currentPage}`;

      if (search) {
        url += `&search=${this.searchTerm}`; // Append the search term to the URL
      }
      axios
        .get(`/admin/horaires?page=${this.currentPage}&search=${this.searchTerm}`, {
          headers: { Authorization: `Bearer ${token.value}` },
        })
        .then((res) => {
          this.totalItems = res.data.abonnesprix.total;
          this.Lignes = res.data.abonnesprix.data.data;
          this.isReady = true;
          this.editDialog = false;
        })
        .catch((err) => {
