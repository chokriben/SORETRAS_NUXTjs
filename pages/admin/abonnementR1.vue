<template>
  <div>
    ...
    <v-table height="650" fixed-header>
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="item in paginatedabonnes" :key="item.id">
          ...
          <td v-if="item.etat !== 'imprimer' && item.profession == null">{{ item.created_at }}</td>
          ...
          <td class="text-center" v-if="item.etat !== 'imprimer' && item.profession == null">
            ...
            <v-btn variant="text" color="blue" fab small @click="printAndMarkAsPrinted(item)">
  ...
  methods: {
   ...
   <div style="display: flex; align-items: center;">
    ...
    async async(search = false) {
      ...
      let url = `/admin/abonnes?page=${this.currentPage}`;
      ...
      axios
        .get(`/admin/abonnes?page=${this.currentPage}&search=${this.searchTerm}&gare_id=${g}`, {
