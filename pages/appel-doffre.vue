...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.$pagename = this.$t("appel_offre.title"); // Mettez à jour la valeur de la variable globale
    ...
    async fetchActualites() {
      try {
        const response = await axios.get(
          "https://api.srtk.com.tn/api/appel_doffres"
        );
        // variable const
        const actualites = response.data.AppelDoffres;
        ...
        this.items = response.data.AppelDoffres; // Stockez les valeurs de actualites dans items
      } catch (error) {
        console.error(error);
        this.snackbar = {
          show: true,
