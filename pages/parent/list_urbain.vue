    ...
    <v-table height="480" fixed-header>
      <thead>
        <tr>
          ...
          <th class="text-center text-uppercase">Nom Parent</th>
          <th class="text-center text-uppercase">Classe</th>
      ...
      <tbody>
        <tr v-for="item in paginatedAbonnes" :key="item.id">
          <td>
            <v-img :aspect-ratio="16 / 6" :src="`https://api.srtk.com.tn/${item.src}`"></v-img>
          </td>
          <td>{{ item.prenom }}</td>
          <td class="text-center">{{ item.nom_parent }}</td>
