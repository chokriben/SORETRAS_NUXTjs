        ...
        <v-hover v-slot="{ isHovering, props }" close-delay="50">
          <v-card :elevation="isHovering ? 16 : 2" :class="{ 'on-hover': isHovering }" class="mx-auto" height="340"
            ...
            <v-card-actions class="mb-2 justify-center">
              <router-link :to="`/actualite-single?id=` + item.id">
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.$pagename = this.$t("actualites.title"); // Mettez à jour la valeur de la variable globale
    ...
    async fetchActualites() {
      try {
        const response = await axios.get(
          "https://api.srtk.com.tn/api/actualites"
        );
        // variable const
